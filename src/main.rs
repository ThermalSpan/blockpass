...
quick_main!(|| -> Result<()> {
    ...
    let mut orig_file = File::open(&original_output_path)
        .chain_err(|| format!("Can't open orig file: {}", original_output_path.display()))?;
    ...
    orig_file.read_to_string(&mut output_buffer)
        .chain_err(|| format!("Can't read contents from {}", original_output_path.display()))?;

    output_file.write_all(output_buffer.as_bytes())
        .chain_err(|| format!("Can't write to {}", args.output.display()))?;

    if ! args.keep_orig {
        fs::remove_file(&original_output_path)
            .chain_err(|| format!("Unable to delete original {}", original_output_path.display()))?;
    }
